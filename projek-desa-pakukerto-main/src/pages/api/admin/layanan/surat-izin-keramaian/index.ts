...
import { updateSuratIzinKeramaianSchema } from "@/schemas/surat-izin-keramaian.schema";
import {
...
export default async function handler(
  ...
  try {
    ...
    if (req.method === "GET") {
      try {
        ...
        let query = db
          ...
          .where("jenisSurat", "==", "surat-izin-keramaian");
        ...
        console.error("Error fetching surat izin keramaian:", error);
        return res
          .status(500)
    ...
    if (req.method === "PUT") {
      ...
      const validationResult = updateSuratIzinKeramaianSchema.safeParse(req.body);
