...
export default function SuratIzinKeramaianAdminPage() {
  ...
  const [selectedSurat, setSelectedSurat] = useState<SuratIzinKeramaian | null>(
    null
  );
  const [nomorSurat, setNomorSurat] = useState("");
  const [newStatus, setNewStatus] = useState<"pending" | "finish">("pending");

  // Query to fetch surat izin keramaian
  const {
    data: suratList,
    isLoading,
    error,
  } = useQuery({
    queryKey: ["surat-izin-keramaian", selectedStatus],
    queryFn: () =>
      suratIzinKeramaianApi.getAll(
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      suratIzinKeramaianApi.update(id, data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => suratIzinKeramaianApi.delete(id),
    onSuccess: () => {
  ...
  const handleViewDetail = (surat: SuratIzinKeramaian) => {
    router.push(`/admin/layanan/surat-izin-keramaian/${surat.id}`);
  };

  const handleEditNomor = (surat: SuratIzinKeramaian) => {
    setSelectedSurat(surat);
    setNomorSurat(surat.nomorSurat || "");
    setIsEditOpen(true);
  };

  const handleEditStatus = (surat: SuratIzinKeramaian) => {
    setSelectedSurat(surat);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Surat Izin Keramaian</h1>
            <p className="text-muted-foreground">
              Kelola permohonan surat izin keramaian dari warga desa
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Daftar Permohonan Surat Izin Keramaian</CardTitle>
            <CardDescription>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Jenis Keramaian</TableHead>
                  ...
                  <TableBody>
                    {suratList && suratList.length > 0 ? (
                      suratList.map((surat: SuratIzinKeramaian) => (
                        <TableRow key={surat.id}>
                          <TableCell className="font-medium">
                            {surat.data.nama}
                          </TableCell>
                          <TableCell>{surat.data.jenisKeramaian}</TableCell>
                          <TableCell>
                            {new Date(surat.data.tanggal).toLocaleDateString(
                              "id-ID"
                            )}
                          </TableCell>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      ...
                                      permohonan surat izin keramaian untuk{" "}
                                      <strong>{surat.data.nama}</strong>?
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="text-center py-8">
                          Belum ada permohonan surat izin keramaian
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
