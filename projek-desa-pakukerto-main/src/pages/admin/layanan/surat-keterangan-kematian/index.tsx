...
export default function SuratKeteranganKematianAdminPage() {
  ...
  const [selectedSurat, setSelectedSurat] =
    useState<SuratKeteranganKematian | null>(null);
  const [nomorSurat, setNomorSurat] = useState("");
  const [newStatus, setNewStatus] = useState<"pending" | "finish">("pending");

  // Query to fetch surat keterangan kematian
  const {
    data: suratList,
    isLoading,
    error,
  } = useQuery({
    queryKey: ["surat-keterangan-kematian", selectedStatus],
    queryFn: () =>
      suratKeteranganKematianApi.getAll(
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      suratKeteranganKematianApi.update(id, data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => suratKeteranganKematianApi.delete(id),
    onSuccess: () => {
  ...
  const handleViewDetail = (surat: SuratKeteranganKematian) => {
    router.push(`/admin/layanan/surat-keterangan-kematian/${surat.id}`);
  };

  const handleEditNomor = (surat: SuratKeteranganKematian) => {
    setSelectedSurat(surat);
    setNomorSurat(surat.nomorSurat || "");
    setIsEditOpen(true);
  };

  const handleEditStatus = (surat: SuratKeteranganKematian) => {
    setSelectedSurat(surat);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">Surat Keterangan Kematian</h1>
            <p className="text-muted-foreground">
              Kelola permohonan surat keterangan kematian dari warga desa
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Daftar Permohonan Surat Keterangan Kematian</CardTitle>
            <CardDescription>
                ...
                <Table>
                  ...
                  <TableBody>
                    {suratList && suratList.length > 0 ? (
                      suratList.map((surat: SuratKeteranganKematian) => (
                        <TableRow key={surat.id}>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              <div>{surat.data.namaPelapor}</div>
                              <div className="text-muted-foreground">
                                {surat.data.nikPelapor}
                              </div>
                            </div>
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              {surat.nomorSurat && (
                                <PDFDownloadLink
                                  document={
                                    <SuratKeteranganKematianPDFDocument
                                      data={surat}
                                    />
                                  }
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      ...
                                      permohonan surat keterangan kematian untuk{" "}
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="text-center py-8">
                          Belum ada permohonan surat keterangan kematian
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
