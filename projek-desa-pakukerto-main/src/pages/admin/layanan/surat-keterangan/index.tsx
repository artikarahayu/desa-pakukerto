...
import { SuratKeteranganPDFDocument } from "@/components/pdf/SuratKeteranganPDFDocument";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { suratKeteranganApi } from "@/lib/admin/surat-keterangan";
import { SuratKeterangan } from "@/schemas/surat-keterangan.schema";
...
export default function SuratKeteranganAdminPage() {
  ...
  const [selectedSurat, setSelectedSurat] = useState<SuratKeterangan | null>(
    null
  );
  const [nomorSurat, setNomorSurat] = useState("");
  const [newStatus, setNewStatus] = useState<"pending" | "finish">("pending");

  // Query to fetch surat keterangan
  const {
    data: suratList,
    isLoading,
    error,
  } = useQuery({
    queryKey: ["surat-keterangan", selectedStatus],
    queryFn: () =>
      suratKeteranganApi.getAll(
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      suratKeteranganApi.update(id, data),
    onSuccess: () => {
      toast.success("Surat berhasil diperbarui");
      queryClient.invalidateQueries({ queryKey: ["surat-keterangan"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => suratKeteranganApi.delete(id),
    onSuccess: () => {
  ...
  const handleViewDetail = (surat: SuratKeterangan) => {
    router.push(`/admin/layanan/surat-keterangan/${surat.id}`);
  };

  const handleEditNomor = (surat: SuratKeterangan) => {
    setSelectedSurat(surat);
    setNomorSurat(surat.nomorSurat || "");
    setIsEditOpen(true);
  };

  const handleEditStatus = (surat: SuratKeterangan) => {
    setSelectedSurat(surat);
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Daftar Permohonan Surat Keterangan</CardTitle>
            <CardDescription>
                ...
                <Table>
                  ...
                  <TableBody>
                    {suratList && suratList.length > 0 ? (
                      suratList.map((surat: SuratKeterangan) => (
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              {surat.nomorSurat && (
                                <PDFDownloadLink
                                  document={
                                    <SuratKeteranganPDFDocument data={surat} />
                                  }
                                  fileName={`surat-keterangan-${surat.data.namaLengkap
                                    .replace(/\s+/g, "-")
                                    .toLowerCase()}.pdf`}
                                >
                                  {({ loading }) => (
                      ...
                      <TableRow>
                        <TableCell colSpan={6} className="text-center py-8">
                          Belum ada permohonan surat keterangan
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
